...
| `nova_memory.py` | 記憶模組，也可獨立執行做記憶管理（見下方說明） |
...
`nova_memory.py` 除了作為記憶模組外，也可以在終端機獨立執行，用來管理、匯出或清除記憶：

**匯出記憶**
不加任何參數直接執行，會將完整記憶匯出成 JSON 檔（`nova_memory_export.json`）：
```bash
...
| 指令 | 用途 | 範例 |
|---|---|---|
| `--list-turns [N]` | 列出最近 N 筆對話與 ID（預設 20 筆） | `python nova_memory.py --list-turns 50` |
| `--search-turns` | 用關鍵字搜尋對話並顯示 ID | `python nova_memory.py --search-turns "口語練習"` |
| `--delete-turns` | 刪除指定 ID 的對話（可傳入多個） | `python nova_memory.py --delete-turns 105 106` |
| `--wipe-conversations`| 僅清空所有「逐字對話」，保留事實與摘要 | `python nova_memory.py --wipe-conversations` |

**使用者事實與摘要管理 (長期/中期記憶)**
| 指令 | 用途 | 範例 |
|---|---|---|
| `--list-facts` | 列出目前記住的所有使用者事實 | `python nova_memory.py --list-facts` |
| `--forget-fact` | 刪除指定的單一事實 | `python nova_memory.py --forget-fact hobby` |
| `--show-summary` | 顯示目前的對話摘要 | `python nova_memory.py --show-summary` |
| `--forget-summary` | 清空目前的對話摘要 | `python nova_memory.py --forget-summary` |

**重置系統**
| 指令 | 用途 | 範例 |
|---|---|---|
| `--wipe-all` | 核彈級全清空：對話、事實、摘要全部歸零，無法復原 | `python nova_memory.py --wipe-all` |
